...
from foundation.models import CropLifeCycleStage


class CropLifeCycleStageFilter(filters.FilterSet):
    def enhanced_type_of_filter(self, name, value):
        ...
        """
        return self.filter(
            Q(type_of=CropLifeCycleStage.TYPE_OF.NONE)|
            Q(type_of=value)
        )
    ...
    class Meta:
        model = CropLifeCycleStage
        fields = ['type_of',]


class CropLifeCycleStageListAPIView(generics.ListAPIView):
    authentication_classes= (OAuth2Authentication,)
    serializer_class = CropLifeCycleStageListSerializer
    ...
    filterset_class = CropLifeCycleStageFilter

    def get_queryset(self):
        """
        Get list data.
        """
        queryset = CropLifeCycleStage.objects.order_by('order_number')
        return queryset
