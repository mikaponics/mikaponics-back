...
from foundation.models import Device, Instrument, TimeSeriesImageDatum
# from foundation.model_resources import create_alert_item_in_system_if_possible

logger = logging.getLogger(__name__)


class TimeSeriesImageDatumCreateSerializer(serializers.Serializer):
    # Primary fields.
    instrument_uuid = serializers.UUIDField(required=True, write_only=True)
    value = Base64ImageField(required=True)
    ...
    def create(self, validated_data):
        ...
        image_value = validated_data.get('value')
        ...
        '''
        # Get our previous record if it exists.
        try:
            previous_time_series_datum = TimeSeriesImageDatum.objects.filter(instrument=instrument).latest('timestamp')
        except TimeSeriesImageDatum.DoesNotExist:
            previous_time_series_datum = None

        # Create our record.
        time_series_datum = TimeSeriesImageDatum.objects.create(
